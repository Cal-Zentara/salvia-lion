...
## What's next
- **CONTENT — next action:** Monday's "3 signs the exhaustion isn't even yours" carousel POSTED June 6 (first in the hybrid lion style, logged as Post 4). This week still needs Wednesday (connect question) + Friday (bath ritual sell) built and posted — both drafted, need saving to a file + slides built. Then write the 6 protection carousels + the faceless videos. Engine for all of it = `salvialion/content-ideas-bank.md`.
0. **$100 ad budget ready to spend** — boost a carousel in the IG app. Settings: women 28-55 US, interests Reiki/energy healing/spirituality/meditation/crystals, $10/day x 10 days, Instagram placement, goal = More messages. (Not yet run.)
1. Embed Nancy's booking link as a "Book a Session" button on the site (timezone fixed + link ready as of Jun 6).
2. Do a test purchase on each of the 6 Stripe links (now live) to confirm money flows
3. Archive old combined Stripe link (`14A5kCchb3usedodqRcbC00`) in Stripe dashboard

## Done
- ✅ **Real White Tea & Lavender bag photo on the site (2026-06-06):** replaced the triple-pack stand-in on that scent with the real labeled bag shot (`pics/WhiteTLavender.jpeg`), baked upright, wired with portrait framing. Live.
- ✅ **Etsy + booking setup (2026-06-06):** Etsy shop payout bank connected via Plaid (under her real name Nancy Huynh). Google Calendar booking timezone fixed to Pacific and booking link ready — just needs embedding on the site as a "Book a Session" button.
- ✅ **First post in the new style is LIVE (2026-06-06):** "3 signs the exhaustion you feel isn't even yours" carousel, hybrid 3D teal/purple lion look, text baked into slides, posted by Cal. Logged as Post 4 in CONTENT-TRACKER. Built a full content engine this session: `content-ideas-bank.md` (7 hook formulas in her voice, ready topic list, the found-message format with 4 variations, and the top rule: lead with the feeling then pick the format/style; the 3D lion is ONE style in a rotation, not the whole look). Locked rules: slide prompts bake text in; never hedge about AI text rendering; site is SalviaLion.com everywhere.
